...
class MetricType(models.Model):
    ...
    unit = models.CharField(max_length=20, choices=UNIT_CHOICES, verbose_name="Единица измерения")
...
class MetricTarget(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="metric_targets")
    metric_type = models.ForeignKey(MetricType, on_delete=models.CASCADE, related_name="targets")
    target_type = models.CharField(max_length=10, choices=TARGET_TYPE_CHOICES, verbose_name="Тип норматива")
    value = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Значение норматива")
...
class DailyMetric(models.Model):
    ...
    metric_type = models.ForeignKey(MetricType, on_delete=models.PROTECT, related_name="daily_values")
    value = models.DecimalField(max_digits=10, null=True, blank=True, decimal_places=2, verbose_name="Значение")
...
class BodyMeasurement(models.Model):
    ...
    visceral_fat = models.DecimalField(
        max_digits=3, decimal_places=1, null=True, blank=True, verbose_name="Висцеральный жир"
    )
    muscle_mass = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True, verbose_name="Мышечная масса (кг)"
    )
    
    # Дополнительные замеры (можно расширять)
    chest = models.DecimalField(max_digits=5, decimal_places=1, null=True, blank=True, verbose_name="Грудь (см)")
    waist = models.DecimalField(max_digits=5, decimal_places=1, null=True, blank=True, verbose_name="Талия (см)")
    hips = models.DecimalField(max_digits=5, decimal_places=1, null=True, blank=True, verbose_name="Бедра (см)")
    biceps = models.DecimalField(max_digits=4, decimal_places=1, null=True, blank=True, verbose_name="Бицепс (см)")
...
class TrainingSession(models.Model):
    ...
    date = models.DateField(db_index=True, verbose_name="Дата")
    training_type = models.CharField(max_length=20, choices=TRAINING_TYPE_CHOICES, verbose_name="Тип тренировки")
...
class ForceMajeure(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="force_majeures")
