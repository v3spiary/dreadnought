...
class MetricTypeTests(TestCase):
    ...
    def test_metric_type_str(self):
        ...
        self.assertEqual(str(self.metric), "Тестовая метрика (г)")
...
class MetricTargetTests(TestCase):
    ...
    def setUp(self):
        ...
        self.metric_type = MetricType.objects.create(
            code="calories", name="Калории", category="nutrition", unit="kcal"
        )
    ...
    def test_metric_target_str(self):
        """Тест строкового представления"""
        self.assertEqual(str(self.target), "testuser - Калории: 2500")
...
class DailyMetricTests(TestCase):
    ...
    def setUp(self):
        ...
        self.metric_type = MetricType.objects.create(
            code="steps", name="Шаги", category="activity", unit="steps"
        )
    ...
    def test_daily_metric_str(self):
        """Тест строкового представления"""
        self.assertEqual(str(self.daily_metric), "testuser - 2026-01-15: Шаги = 10000")
...
class BodyMeasurementTests(TestCase):
    ...
    def test_body_measurement_str(self):
        """Тест строкового представления"""
        self.assertEqual(str(self.measurement), "testuser - 2026-01-15: 80.5кг")
    ...
    def test_optional_fields(self):
        ...
        measurement = BodyMeasurement.objects.create(
            user=self.user, date=date(2026, 1, 16), weight=Decimal("80.0")
        )
...
class TrainingSessionTests(TestCase):
    ...
    def test_training_session_str(self):
        """Тест строкового представления"""
        self.assertEqual(str(self.training), "testuser - 2026-01-15: Силовая")
...
class APITests(TestCase):
    ...
    def test_update_metrics_single(self):
        ...
        daily_metric = DailyMetric.objects.get(
            user=self.user, metric_type=self.metric_type
        )
    ...
    def test_update_metrics_invalid_metric(self):
        ...
        url = reverse("metrics-update")
        data = {"metrics": {"invalid_metric": 1000}}
        response = self.client.patch(url, data, format="json")
    ...
    def test_get_body_latest(self):
        ...
        # Создаем замеры
        BodyMeasurement.objects.create(
            user=self.user, date=date(2026, 1, 14), weight=81.0
        )
        BodyMeasurement.objects.create(
            user=self.user, date=date(2026, 1, 15), weight=80.5
        )
    ...
    def test_get_dashboard_today(self):
        ...
        today = timezone.now().date()
        DailyMetric.objects.create(
            user=self.user, date=today, metric_type=self.metric_type, value=8000
        )
...
class ManagementCommandsTests(TestCase):
    ...
    def test_init_tracker_force(self):
        ...
        MetricType.objects.create(
            code="calories", name="Старые калории", category="old", unit="old_unit"
        )
...
class EdgeCaseTests(TestCase):
    ...
    def test_update_metrics_null_value(self):
        ...
        metric_type = MetricType.objects.create(
            code="water", name="Вода", category="nutrition", unit="l"
        )

        url = reverse("metrics-update")
        data = {"metrics": {"water": None}}
        response = self.client.patch(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # Проверяем, что метрика создалась с null значением
        daily_metric = DailyMetric.objects.get(user=self.user, metric_type=metric_type)
    ...
    def test_update_metrics_future_date(self):
        ...
        url = reverse("metrics-update")
        data = {"date": future_date, "metrics": {"steps": 10000}}
        response = self.client.patch(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_get_metrics_period_large_range(self):
        ...
        response = self.client.get(
            url, {"start_date": "2023-01-01", "end_date": "2026-12-31"}  # > 90 дней
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_create_body_measurement_future_date(self):
        ...
        url = reverse("body-create")
        data = {"date": future_date, "weight": 80.0}
        response = self.client.post(url, data, format="json")
    ...
    def test_duplicate_body_measurement(self):
        ...
        BodyMeasurement.objects.create(
            user=self.user, date=date(2026, 1, 15), weight=80.0
        )

        url = reverse("body-create")
        data = {"date": "2026-01-15", "weight": 81.0}
        response = self.client.post(url, data, format="json")
