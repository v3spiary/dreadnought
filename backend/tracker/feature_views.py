...
class BodyMeasurementListView(APIView):
    ...
    def get(self, request):
        ...
        measurements = BodyMeasurement.objects.filter(user=request.user).order_by(
            "-date"
        )[offset : offset + limit]
...
class BodyMeasurementLatestView(APIView):
    ...
    def get(self, request):
        measurement = (
            BodyMeasurement.objects.filter(user=request.user).order_by("-date").first()
        )
...
class BodyMeasurementCreateView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            # Проверяем дату
            measurement_date = serializer.validated_data.get(
...
class TrainingSessionByDateView(APIView):
    ...
    def get(self, request, date_str):
        ...
        trainings = TrainingSession.objects.filter(user=request.user, date=target_date)
...
class AnalyticsTrendView(APIView):
    ...
    def get(self, request, metric_code):
        ...
        metric_type = get_object_or_404(MetricType, code=metric_code, is_active=True)
        ...
        for metric in metrics:
            values.append({"date": metric.date, "value": float(metric.value)})

        data = {"metric": metric_code, "unit": metric_type.unit, "values": values}
